...
Current open question: how important is having both barcode scanning and machine learning? Some items do not have barcodes, so it's clear that barcode scanning is not sufficient. But is it needed at all? Should everything just be video recognition? I think we don't want videos to be stored locally. So AI would have to save metadata based on the video, such that it could recognise if a future video relates to the same product. 
...
To test, I have used a barcode scanner on supermarket items I have bought, and used an open source api: `curl https://world.openfoodfacts.org/api/v0/product/BARECODE.json`. This seems to work! But requires internet. 
